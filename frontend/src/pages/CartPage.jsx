import React from 'react'
import { useCart } from '../context/CartContext';
import { Link } from 'react-router-dom';

const CartPage = () => {

    const { cartItems, removeFromCart, total, updateQuantity } = useCart();
    const BASEURL = import.meta.env.VITE_DJANGO_BASE_URL
    console.log("Car Items: ",cartItems)

    return (
        <div className='pt-20 min-h-screen bg-gray-100 p-8'>
            <h1 className='text-3xl font-bold text-center mb-6'>🛒 Your Cart</h1>
            {cartItems.length === 0 ? (
                <p className='text-center text-red-400'>🤬 Your Cart is Empty</p>
            ) : (
                <div className='max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md'>
                    {
                        cartItems.map((item) => (
                            <div key={item.id} className='flex items-center justify-between mb-4'>
                                <div className='flex items-center gap-4'>
                                    {
                                        item.product_image && (
                                            <img src={`${BASEURL}${item.product_image}`} alt={item.product_name} className='w-20 h-20 object-cover rounded' />
                                        )
                                    }
                                </div>
                                <div>
                                    <h2 className='text-lg font-semibold'>{item.product_name}</h2>
                                    <h2 className='text-gray-600'>${item.product_price}</h2>
                                </div>
                                <div className='flex items-center gap-3'>
                                    <button onClick={() => updateQuantity(
                                        item.id,
                                        item.quantity - 1
                                    )} className='bg-green-300 px-3 py-1 rounded'>-</button>
                                    <span>{item.quantity}</span>
                                    <button onClick={() => updateQuantity(
                                        item.id,
                                        item.quantity + 1
                                    )} className='bg-green-300 px-3 py-1 rounded'>+</button>
                                    <button onClick={() => removeFromCart(
                                        item.id
                                    )} className='text-red-500'>Remove</button>
                                </div>
                            </div>
                        ))
                    }
                    <div className='border-t flex justify-between items-center pt-4 mt-4 '>
                        <h2 className='text-xl font-bold'>Total:</h2>
                        <p className='text-xl font-semibold'>${total.toFixed(2)}</p>
                        <Link to="/checkout" className='bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition duration-300'>
                        Proceed to Checkout
                        </Link>
                    </div>
                </div>
            )}
        </div>
    )
}

export default CartPage
